...
> Note: `vecA[i]!` uses the non-null assertion (`!`) because TypeScript thinks the array might have a hole. For our case the array is dense, so `!` is safe. If you prefer to be defensive, write `vecA[i] ?? 0` instead.
