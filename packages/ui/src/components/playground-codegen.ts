import { SAMPLE_EXPENSES_CSV } from './playground-table.js';
...
// The same sample data the playground's Read spreadsheet node parses; swap
// this for a real fs.readFileSync(...) once you're pointing at your own file.
const SAMPLE_EXPENSES_CSV = ${jsString(SAMPLE_EXPENSES_CSV)};
...
export function generateStandaloneScript(workflow: SavedWorkflow): string {
  ...
  for (const node of order) {
    ...
    switch (node.kind) {
      case 'read-file': {
        lines.push(`  const ${varName} = parseCsv(SAMPLE_EXPENSES_CSV);`);
        lines.push(`  out[${jsString(node.id)}] = ${varName};`);
        lines.push(`  console.log(${f('sheet') || jsString('Expenses')} + ':', ${varName}.rows.length, 'rows');`);
        break;
      }
      case 'filter': {
