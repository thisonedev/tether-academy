...
import type { CurriculumChapter } from '@academy/courses';
...
export function ChapterLandingBody({ chapter }: { chapter: CurriculumChapter }) {
  ...
  const isChapterDone =
    shipped.length > 0 && shipped.every((l) => doneKeys.has(`${chapter.slug}-${l.slug}`));
  ...
  return (
    <div className="prose-md">
      ...
      <p className="text-canvas-muted-foreground">
        {chapter.lessons.length} {chapter.lessons.length === 1 ? 'lesson' : 'lessons'} in this
        chapter.
        {isChapterDone
          ? ' All shipped lessons done — +50 XP earned.'
          : ' Pick one below or jump to the first shipped lesson.'}
      </p>
