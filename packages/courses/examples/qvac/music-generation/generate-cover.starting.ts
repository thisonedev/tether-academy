...
async function main() {
  ...
  // 1: call audioGen() with taskType: "cover-nofsq", sourceAudio: sourcePath, audioCoverStrength, coverNoiseStrength, and a seed

  // 2: drain progressStream, then Promise.all([run.audio, run.stats])

  // 3: wrap audio.pcm in a WAV header (createWav is prefilled below) and write to "output/music-generation/audiogen-cover-output.wav"
